...
🧠 4Q Assists: {assists['name']} ({assists['team']}) – {assists['stat']} AST"""

    if team_4q_diff:
        team_name = TEAM_NAME_MAP.get(team_4q_diff[0], team_4q_diff[0])
        tweet += f"\n📈 Best 4Q Team: {team_name} +{team_4q_diff[1]}"
...
def run_bot():
    ...
    try:
        ...
        if not game_ids or not game_number:
            print("No Finals games found for", date_str)
            return
        ...
        tweet = compose_finals_clutch_tweet(date_str, game_number, points, fg, assists, team_4q_diff)
