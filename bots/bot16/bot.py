...
def get_finals_game_ids_and_number(date_str, max_retries=3):
    for attempt in range(max_retries):
        try:
            ...
            # Assume only Finals games are happening now — grab all games
...
def get_efficiency_stats(game_ids):
    ...
    for game_id in game_ids:
        ...
        for p in players:
            ...
            double_digits = sum(1 for stat in [points, rebounds, assists, steals, blocks] if stat >= 10)
            if double_digits >= 3:
                triple_double = {
                    "name": name,
                    "team": team,
...
def compose_finals_efficiency_tweet(date_str, game_number, efficiency, plus_minus, stocks, triple_double):
    formatted_date = datetime.strptime(date_str, "%Y-%m-%d").strftime("%m/%d/%Y")

    tweet = f"""🏆 NBA Finals – Game {game_number}  
📅 {formatted_date}

⚡ Efficient: {efficiency['name']} ({efficiency['team']}) – {efficiency['fg_pct']}% FG ({efficiency['fga']} FGA)
📈 Plus/Minus: {plus_minus['name']} ({plus_minus['team']}) – +{plus_minus['plus_minus']}
🛡️ Defense: {stocks['name']} ({stocks['team']}) – {stocks['stocks']} STL+BLK"""

    if triple_double:
        tweet += f"""\n\n👑 Triple-Double: {triple_double['name']} ({triple_double['team']}) – {triple_double['pts']} PTS, {triple_double['reb']} REB, {triple_double['ast']} AST"""
...
def run_bot():
    ...
    try:
        game_ids, game_number = get_finals_game_ids_and_number(date_str)
        if not game_ids or not game_number:
            print("No Finals game found for", date_str)
            return

        efficiency, plus_minus, stocks, triple_double = get_efficiency_stats(game_ids)
        tweet = compose_finals_efficiency_tweet(date_str, game_number, efficiency, plus_minus, stocks, triple_double)
        print("Tweeting:\n", tweet)
